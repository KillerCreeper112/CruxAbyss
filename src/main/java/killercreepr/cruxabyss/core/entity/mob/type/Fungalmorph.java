...
import killercreepr.cruxabyss.core.entity.mob.goal.FungalmorphGoal;
...
public class Fungalmorph extends SimpleAbyssMob {

    public Fungalmorph() {
        super(Crux.key("fungalmorph"), EntityType.PIG);
    }

    @Override
    public @Nullable Consumer<Entity> spawnFunction(@Nullable AbyssWorld world, @NotNull Location l) {
        return e ->{
            e.customName(Component.text("Fungalmorph"));
            ...
            if(e instanceof LivingEntity ee){
                double movement = ee.getAttribute(Attribute.MOVEMENT_SPEED).getBaseValue() * 1.2D;
    ...
    public @Nullable Map<CruxAttribute, Collection<CruxAttributeModifier>> getAttributes(@Nullable AbyssWorld world, @NotNull Entity e) {
        ...
        addAttribute(map, CruxAttribute.ATTACK_DAMAGE,
                CruxAttributeModifier.baseModifier(CruxMath.random(5D, 7D) * (world == null ? 1D : world.getDifficulty())));
